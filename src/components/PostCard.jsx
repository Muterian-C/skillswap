...
const PostCard = ({ post, onDelete, onLike, onReply, currentUser, isInPostPage = false }) => {
  ...
  }, [currentUser, post.liked_by_current_user, post.likes_count]);
  ...
  const handlePostClick = () => {
    if (isInPostPage) return; // Don't navigate if we're already on the post page
  ...
  return (
    <div 
      className={`post-card bg-white rounded-xl shadow-sm p-5 mb-5 border border-gray-100 hover:shadow-md transition-all duration-300 ${!isInPostPage ? 'cursor-pointer' : ''} ${isClicked ? 'scale-95 opacity-90' : ''}`}
      onClick={!isInPostPage ? handlePostClick : undefined}
      onMouseDown={() => !isInPostPage && setIsClicked(true)}
      onMouseUp={() => !isInPostPage && setIsClicked(false)}
      onMouseLeave={() => !isInPostPage && setIsClicked(false)}
    >
      <div className="flex">
